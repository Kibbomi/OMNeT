...
#include "RSUClusterAppOptimal.h"
...
Define_Module(RSUClusterAppOptimal);

//추가
void RSUClusterAppOptimal::initialize(int stage)
{
...
void RSUClusterAppOptimal::finish()
{
...
void RSUClusterAppOptimal::onWSA(DemoServiceAdvertisment* wsa)
{
...
void RSUClusterAppOptimal::onWSM(BaseFrame1609_4* frame)
{
    ...
    if(strcmp(pac->getName(),"CarCOReq") == 0){
        ...
        FindOptimalES(msg->getRequiredCycle(), msg->getReqTime() + msg->getConstraint());
...
//HandleMessage
void RSUClusterAppOptimal::handleMessage(cMessage* msg)
{
...
//override함.
void RSUClusterAppOptimal::handleSelfMsg(cMessage* msg)
{
    if(msg->getKind() == Self_ERSReq ){
        ...
        if(TTL_init + TTL_increasement <= TTL_threshold)
            ...
            EV_INFO << "RSUClusterAppOptimal : TTL value exceeds the TTL_threshold value. \n";

    }
    else if(msg->getKind() == Self_queuing)
    {
        if(!q.empty()){
            FindOptimalES(q.front().getRequiredCycle(), q.front().getRequiredCycle() + q.front().getConstraint());
...
void RSUClusterAppOptimal::socketDataArrived(inet::Ieee8022LlcSocket *socket, inet::Packet *msg)
{
...
void RSUClusterAppOptimal::socketClosed(inet::Ieee8022LlcSocket *socket)
{
...
//implementation EPS
void RSUClusterAppOptimal::BeginERS(int init, int threshold){
...
void RSUClusterAppOptimal::FindOptimalES(double requiredCycle, simtime_t constraint){
    EV<<this->getParentModule()->getFullName()<<" call FindOptimalES\n";
    //myOptimal 갱신이 필요한 것.
    inet::Format_EdgeServer OptimalES;
    OptimalES.f = INITVAL;    //기본생성자가 실행이 안 됨.   명시적 초기화.
    OptimalES.isAvailable = false;
    ...
    for(auto iter = ESs.begin(); iter!=ESs.end(); ++iter){
        ...
        simtime_t evalTime = simTime() + requiredCycle / iter->second.f + propagationDelay; //20ms

        if((*iter).second.isAvailable && evalTime < constraint && (*iter).second.f < OptimalES.f )
        {
...
void RSUClusterAppOptimal::FindOptimalES(){
    EV<<this->getParentModule()->getFullName()<<" call FindOptimalES\n";
    //myOptimal 갱신이 필요한 것.
    inet::Format_EdgeServer OptimalES;
...
void RSUClusterAppOptimal::SendRSUCOLevel(bool level)
{
    return ;
