...
void VehicleBaseAppLayer::initialize(int stage)
{
    VehicleBaseAppLayer::initialize(stage);
...
simtime_t VehicleBaseAppLayer::computeAsynchronousSendingTime(simtime_t interval, ChannelType chan)
{
...
void VehicleBaseAppLayer::populateWSM(BaseFrame1609_4* wsm, LAddress::L2Type rcvId, int serial)
{
    wsm->setRecipientAddress(rcvId);
    wsm->setBitLength(headerLength);

    if (RSUBeacon* bsm = dynamic_cast<RSUBeacon*>(wsm)) {
...
void VehicleBaseAppLayer::receiveSignal(cComponent* source, simsignal_t signalID, cObject* obj, cObject* details)
{
    Enter_Method_Silent();
...
void VehicleBaseAppLayer::handlePositionUpdate(cObject* obj)
{
...
void VehicleBaseAppLayer::handleParkingUpdate(cObject* obj)
{
    isParked = mobility->getParkingState();
}

void VehicleBaseAppLayer::handleLowerMsg(cMessage* msg)
{
...
void VehicleBaseAppLayer::handleSelfMsg(cMessage* msg)
{
    switch (msg->getKind()) {
    case SEND_BEACON_EVT: {
        RSUBeacon* bsm = new RSUBeacon();
        populateWSM(bsm);
        sendDown(bsm);
...
void VehicleBaseAppLayer::finish()
{
...
VehicleBaseAppLayer::~VehicleBaseAppLayer()
{
...
void VehicleBaseAppLayer::startService(Channel channel, int serviceId, std::string serviceDescription)
{
...
void VehicleBaseAppLayer::stopService()
{
    cancelEvent(sendWSAEvt);
    currentOfferedServiceId = -1;
}

void VehicleBaseAppLayer::sendDown(cMessage* msg)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDown(msg);
}

void VehicleBaseAppLayer::sendDelayedDown(cMessage* msg, simtime_t delay)
{
    checkAndTrackPacket(msg);
    BaseApplLayer::sendDelayedDown(msg, delay);
}

void VehicleBaseAppLayer::checkAndTrackPacket(cMessage* msg)
{
    if (dynamic_cast<RSUBeacon*>(msg)) {
